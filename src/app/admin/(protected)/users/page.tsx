        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          ...
          <tbody>
            {users.map(user => {
              ...
              const canDelete = !isSuper && (isSuperAdmin || isCurrentUser);

              return (
                <tr key={user.id} style={{ borderBottom: '1px solid #eee' }}>
                  <td style={{ padding: '1rem', fontWeight: 500 }}>
                    {user.name}
                    {isCurrentUser && <span style={{ marginLeft: '0.5rem', fontSize: '0.75rem', backgroundColor: '#e8f5e9', color: '#2e7d32', padding: '0.1rem 0.4rem', borderRadius: '10px' }}>You</span>}
                  </td>
                  <td style={{ padding: '1rem', color: '#555' }}>{user.email}</td>
                  <td style={{ padding: '1rem' }}>
                    {isSuper ? (
                      <span style={{ backgroundColor: '#fff8e1', color: '#f57f17', padding: '0.2rem 0.6rem', borderRadius: '4px', fontSize: '0.85rem', fontWeight: 500 }}>Super Admin</span>
                    ) : (
                      <span style={{ backgroundColor: '#e3f2fd', color: '#1565c0', padding: '0.2rem 0.6rem', borderRadius: '4px', fontSize: '0.85rem', fontWeight: 500 }}>Admin</span>
                    )}
                  </td>
